...
class Staff_model extends CI_Model {
   ...
   {
    $query=$this->db->select('staff.id as sid,staff.name,staff_leave_log.user_id,staff_leave_log.user_id,staff_leave_log.action,staff_leave_log.old_alloted_leave,staff_leave_log.new_alloted_leave,staff_leave_details.staff_id,staff_leave_details.id as slid,leave_types.id as lid,leave_types.type')->from('staff_leave_log')->join('staff','staff.id =staff_leave_log.staff_id')->join('staff_leave_details','staff_leave_details.id=staff_leave_log.leave_id')->join('leave_types','leave_types.id=staff_leave_log.leave_type_id')->get()->result_array();

    // echo $this->db->last_query();exit;
    return $query;
   }
   public function getstaffleavereport($staff_list,$type)
   {
    ...
    if(empty($type))
    {
        ...
        leave_types.type,staff_leave_log.old_alloted_leave,staff_leave_log.new_alloted_leave')
    ->from('staff_leave_details')
    ->join('staff', 'staff.id = staff_leave_details.staff_id')
    ->join('staff_leave_log', 'staff_leave_details.id = staff_leave_log.leave_id')
    ->join('leave_types', 'leave_types.id = staff_leave_log.leave_type_id')
    ->where('staff.id', $staff_list)
...
//    public function staff_leave_list()
//    {
//     $query=$this->db->select('staff_leave_details.id,staff_leave_details.staff_id,staff_leave_details.leave_type_id,staff_leave_details.alloted_leave,staff_leave_details.description,staff.name,staff.id as sid,leave_types.id as ltid,leave_types.type')->from('staff_leave_details')->join('staff','staff.id=staff_leave_details.staff_id')->join('leave_types','staff_leave_details.leave_type_id=leave_types.id')->get()->result_array();
//     // echo $this->db->last_query();exit;
//     return $query;
//    }
    ...
    public function addleave($data)
    {
        if (isset($data['staff_id'])) {
            ...
            if ($query->num_rows() > 0) {
                ...
                $new_alloted_leave = $existing['alloted_leave'] + $data['alloted_leave'];
        ...
        $this->db->insert('staff_leave_details', $data);
        return $this->db->insert_id();
    }
