...
class Staff extends Admin_Controller {
    ...
    function leave_report()
    {
        ...
        $this->form_validation->set_rules('staff_list', 'Staff Name', 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {

            $this->load->view("layout/header", $data);
            $this->load->view("admin/staff/leave_report", $data);
            $this->load->view("layout/footer", $data);
        } else {

            $result = $this->staff_model->getstaffleavereport($staff_list, $type);
            $data["result"] = $result;
            $this->load->view("layout/header", $data);
            $this->load->view("admin/staff/leave_report", $data);
            $this->load->view("layout/footer", $data);
    ...
    function staff_leave()
    {
        ...
        $leavetype = $this->staff_model->getLeaveType();
        $data['leavetype'] = $leavetype;
        $data['leave_list'] = $this->staff_model->staff_leave_list();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $previous_leave = $this->db->get('staff_leave_details')->row();

            $old_alloted_leave = $previous_leave ? $previous_leave->alloted_leave : 0;

            $staff_leave = array(
                'staff_id'       => $staff_id,
                'leave_type_id'  => $leave_type_id,
                'alloted_leave'  => $new_alloted_leave,
    ...
    {
        if (!$this->rbac->hasPrivilege('add_staff_leave', 'can_edit')) {
            access_denied();
        }
        // echo $id;
        $this->form_validation->set_rules('staff', 'Staff', 'required');
